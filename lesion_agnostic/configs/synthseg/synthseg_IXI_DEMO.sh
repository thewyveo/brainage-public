#!/bin/bash -l
#SBATCH --job-name=ixi_synthseg
#SBATCH --partition=gpu_a100
#SBATCH --gpus=1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=8
#SBATCH --mem=64G
#SBATCH --time=24:00:00
#SBATCH --output=/home/kozdemir/logs/ixi_synthseg_%j.out
#SBATCH --error=/home/kozdemir/logs/ixi_synthseg_%j.err

set -xo pipefail

REPO="/home/kozdemir/brainage/lesion_agnostic/exp_1/inpainting/USB_og/USB/assets"

module purge
module load 2023

export FREESURFER_HOME="/home/kozdemir/freesurfer"
export SUBJECTS_DIR="$FREESURFER_HOME/subjects"
export FUNCTIONALS_DIR="$FREESURFER_HOME/sessions"
mkdir -p "$SUBJECTS_DIR" "$FUNCTIONALS_DIR"

mkdir -p /home/kozdemir/logs

set +e
source "$FREESURFER_HOME/SetUpFreeSurfer.sh"
FS_SETUP_STATUS=$?
set -e

if [ "$FS_SETUP_STATUS" -ne 0 ]; then
  echo "WARNING: FreeSurfer setup returned $FS_SETUP_STATUS, continuing anyway."
fi

echo "HOST=$(hostname)"
echo "REPO=$REPO"
echo "FREESURFER_HOME=$FREESURFER_HOME"
echo "SUBJECTS_DIR=$SUBJECTS_DIR"
date

which mri_synthseg
mri_synthseg --help | head -20

cd "$REPO"
mkdir -p usb_synthseg_labels

mri_synthseg \
  --i "/home/kozdemir/IXI_n4_rigid_first10" \
  --o "/home/kozdemir/IXI_synthseg_DEMO10" \
  --robust

find "$REPO/usb_synthseg_labels" -type f -ls

echo "DONE"
date
