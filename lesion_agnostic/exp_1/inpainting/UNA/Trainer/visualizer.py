...
class BaseVisualizer(object):
    ...
    def __init__(self, gen_args, train_args, draw_border=False): 
        ...
        if 'registration' in self.tasks or 'bias_field' in self.tasks:
...
class TaskVisualizer(BaseVisualizer):
    ...
    def visualize_all(self, subjects, samples, outputs, out_dir, output_names = ['image'], target_names = ['image']):
        ...
        names = [name.split('.nii')[0] for name in subjects['name']]
        input_name = 'source' if 'registration' in self.tasks else 'input'
        ...
        for i, name in enumerate(names): 
            ...
            if len(out_images) > 0:
                curr_target = {} 
                # two target name independent of self.subject_robust
                ...
                if not self.subject_robust:
                    for target_name in target_names:  
                        if target_name in samples[0] and target_name not in curr_target.keys():
                            try:
