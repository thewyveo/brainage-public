...
class AbstractUNetSep(nn.Module):
    """
    ...
    def __init__(self, in_channels, basic_module, f_maps=64, layer_order='gcr',
        ...
        super(AbstractUNetSep, self).__init__()
        ...
        # create decoder path #1
        self.decoders_normal = create_decoders(self.f_maps, basic_module, conv_kernel_size, conv_padding, layer_order, num_groups,
                                        is_3d)
        # create decoder path #2
        self.decoders_pathol = create_decoders(self.f_maps, basic_module, conv_kernel_size, conv_padding, layer_order, num_groups,
                                        is_3d)
    ...
    def forward(self, x):
        # encoder part
        encoders_features = [] 
        for i, encoder_a in enumerate(self.encoders):
            x = encoder_a(x) 
        ...
        x_normal, x_pathol = x.clone(), x.clone()

        # decoder part
        for decoder_normal, decoder_pathol, encoder_features in zip(self.decoders_normal, self.decoders_pathol, encoders_features):
            # pass the output from the corresponding encoder and the output
            # of the previous decoder
            x_normal = decoder_normal(encoder_features, x_normal)
            x_pathol = decoder_pathol(encoder_features, x_pathol)
        
        if self.is_unit_vector:
            x_normal = F.normalize(x_normal, dim=1)
            x_pathol = F.normalize(x_pathol, dim=1)

        return {'normal': x_normal, 'pathology': x_pathol}
    ...
    def get_feature(self, x):  
        ...
        x_normal, x_pathol = x.clone(), x.clone() 
        decoders_normal_features, decoders_pathol_features = [x_normal], [x_pathol]
        for decoder_normal, decoder_pathol, encoder_features in zip(self.decoders_normal, self.decoders_pathol, encoders_features):
            # pass the output from the corresponding encoder and the output
            # of the previous decoder
            x_normal = decoder_normal(encoder_features, x_normal)
            x_pathol = decoder_normal(encoder_features, x_pathol)
            decoders_normal_features.append(x_normal) 
            decoders_pathol_features.append(x_pathol)
        if self.is_unit_vector:
            decoders_normal_features[-1] = F.normalize(decoders_normal_features[-1], dim=1)
            decoders_pathol_features[-1] = F.normalize(decoders_pathol_features[-1], dim=1)
        return {'normal': decoders_normal_features, 'pathology': decoders_pathol_features}
...
class UNet3DSep(AbstractUNetSep):
    """
    3DUnet model from
    `"3D U-Net: Learning Dense Volumetric Segmentation from Sparse Annotation"
        <https://arxiv.org/pdf/1606.06650.pdf>`.
    ...
    def __init__(self, in_channels, f_maps, layer_order='gcl', num_groups=8, num_levels=5, is_unit_vector=False, conv_padding=1, **kwargs):
        ...
        super(UNet3DSep, self).__init__(in_channels=in_channels,
                                     basic_module=DoubleConv,
                                     f_maps=f_maps,
                                     layer_order=layer_order,
                                     num_groups=num_groups,
