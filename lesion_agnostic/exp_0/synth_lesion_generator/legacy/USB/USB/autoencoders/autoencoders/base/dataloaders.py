...
class AEDataModule(pl.LightningDataModule):
    """LightningDataModule for autoencoder data.
    ...
    def setup(self, stage):
        if self.is_validate:
            ...
            self.train_dataset = hydra.utils.instantiate(self.dataset, data=train_data, labels=train_labels)
            self.test_dataset = hydra.utils.instantiate(self.dataset, data=test_data, labels=test_labels)
        else:
            self.train_dataset = hydra.utils.instantiate(self.dataset, data=self.data, labels=self.labels) 
    ...
    def train_dataloader(self): #TODO fix num_workers
        return DataLoader(self.train_dataset, batch_size=self.batch_size, shuffle=False, num_workers=3) #use default num_workers for now, problem in windows! 
