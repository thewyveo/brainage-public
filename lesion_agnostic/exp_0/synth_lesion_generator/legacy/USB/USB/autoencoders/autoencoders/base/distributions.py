...
class Normal(Normal):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if isinstance(self.loc, list) or isinstance(self.loc, ListConfig):
            self.loc = torch.tensor(self.loc, dtype=torch.float32)
        if isinstance(self.scale, list) or isinstance(self.scale, ListConfig):
