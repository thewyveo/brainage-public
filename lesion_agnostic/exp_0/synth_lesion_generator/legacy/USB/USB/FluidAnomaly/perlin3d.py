...
def generate_perlin_noise_3d(
        ...
        interpolant=interpolant, percentile=None, seed = None
):
    ...
    delta = (shape[0] // res[0], shape[1] // res[1], shape[2] // res[2])
...
def generate_shape_3d(shape, perlin_res, percentile, device, seed = None):
    pprob, p = generate_perlin_noise_3d(shape, perlin_res, tileable=(True, False, False), percentile=percentile, seed=seed) 
    #return torch.from_numpy(p).to(device), torch.from_numpy(pprob).to(device)
    return torch.from_numpy(p), torch.from_numpy(pprob)


def generate_velocity_3d(shape, pad_shape, perlin_res, V_multiplier, device, save_orig_for_visualize = False, seed = None): 
    # Ensure pad_shape is a multiple of 2 and larger than or equal to shape
    pad_shape = [(x + 1) // 2 * 2 if x % 2 != 0 else x for x in shape]
    pad_shape = [max(pad, shape_dim) for pad, shape_dim in zip(pad_shape, shape)]

    #print('pad ', shape, '-->', pad_shape)

    # generate random potentials (back to original shape)
    curl_a = generate_perlin_noise_3d(pad_shape, perlin_res, tileable=(True, False, False), seed=seed) 
    curl_b = generate_perlin_noise_3d(pad_shape, perlin_res, tileable=(True, False, False), seed=seed) 
    curl_c = generate_perlin_noise_3d(pad_shape, perlin_res, tileable=(True, False, False), seed=seed) 

    curl_a = crop_to_shape(curl_a, shape, pad_shape)
    curl_b = crop_to_shape(curl_b, shape, pad_shape)
    curl_c = crop_to_shape(curl_c, shape, pad_shape)

    # Generate velocity fields
    Vx, Vy, Vz = stream_3D(
        torch.from_numpy(curl_a), #.to(device),
    ...
    if save_orig_for_visualize:
        return {
            "Vx": (Vx * V_multiplier),
            "Vy": (Vy * V_multiplier), #.to(device),
            "Vz": (Vz * V_multiplier),
            "curl_a": curl_a,
            "curl_b": curl_b,
            "curl_c": curl_c,
        }
    return {"Vx": (Vx * V_multiplier), "Vy": (Vy * V_multiplier), "Vz": (Vz * V_multiplier)}




# Crop back to original shape
def crop_to_shape(array, shape, pad_shape):
    return array[
        (pad_shape[0] - shape[0]) // 2 : (pad_shape[0] - shape[0]) // 2 + shape[0],
