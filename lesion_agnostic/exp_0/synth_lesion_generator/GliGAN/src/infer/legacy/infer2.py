...
def failed_attempt_dir_for(healthy_path: Path, label_path: Path, attempt_idx: int, output_dir: Path) -> Path:
    stem_h = sanitize_name(strip_nii(healthy_path.name).replace("_preprocessed", ""))
    stem_l = sanitize_name(strip_nii(label_path.name).replace("-seg", ""))
    return output_dir / "_failed_attempts" / f"{stem_h}__{stem_l}__attempt_{attempt_idx:03d}"
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "T1-only GliGAN inference with strict unique IXI/unique BraTS pairing. "
            "If a BraTS mask fails for an IXI, the script consumes that failed mask "
            "and retries the same IXI with the next unused mask."
        )
    )
    ...
    parser.add_argument(
        ...
        default="shuffle",
        help="shuffle = random one-to-one order using seed. sorted = deterministic sorted healthy/mask order.",
    )
    parser.add_argument(
        "--max-mask-attempts-per-ixi",
        type=int,
        default=0,
        help="0 means keep trying unused masks until this IXI succeeds or masks run out.",
    )
    parser.add_argument(
...
def main():
    ...
    log_path = args.output_dir / f"gligan_inference_log_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
    ...
    failed_csv = args.output_dir / "failed_mask_attempts.csv"


    try:
        log("=" * 80)
        log("GliGAN unique-pair retry inference")
        log("=" * 80)
        ...
        if len(label_files) < len(healthy_files):
            log("WARNING: fewer masks than IXI MRIs. Some IXIs may fail if retries consume masks.")


        if args.pairing_mode == "shuffle":
        ...
        # Save initial ordered queues for reproducibility.
        initial_queue_csv = args.output_dir / "initial_ordered_queues.csv"
        max_len = max(len(healthy_files), len(label_files))
        with open(initial_queue_csv, "w", encoding="utf-8", newline="") as f:
            writer = csv.DictWriter(f, fieldnames=["index", "healthy_path", "label_path"])
            writer.writeheader()
            for idx in range(max_len):
                writer.writerow({
                    "index": idx,
                    "healthy_path": str(healthy_files[idx]) if idx < len(healthy_files) else "",
                    "label_path": str(label_files[idx]) if idx < len(label_files) else "",
                })
        log(f"Saved initial ordered queues: {initial_queue_csv}")
        ...
        for healthy_idx, healthy_path in enumerate(healthy_files, start=1):
            ...
            log("=" * 80)
            log(f"[IXI {healthy_idx}/{len(healthy_files)}] {healthy_path.name}")
            log("=" * 80)
            ...
            try:
                ...
                failed_ixis += 1
                err = f"FAILED_TO_LOAD_HEALTHY: {repr(e)}"
                log(err)
                continue


            success_for_this_ixi = False
            attempt_for_healthy = 0


            while not success_for_this_ixi:
                if label_cursor >= len(label_files):
                    log(f"NO_UNUSED_MASKS_LEFT for {healthy_path.name}")
                    failed_ixis += 1
                    break


                if args.max_mask_attempts_per_ixi > 0 and attempt_for_healthy >= args.max_mask_attempts_per_ixi:
                    log(f"MAX_ATTEMPTS_REACHED for {healthy_path.name}")
                    failed_ixis += 1
                    break


                label_global_index = label_cursor + 1
                label_path = label_files[label_cursor]
                label_cursor += 1


                attempt_for_healthy += 1
                label_stem = strip_nii(label_path.name).replace("-seg", "")
                case_dir = case_dir_for(healthy_path, label_path, args.output_dir)


                log(f"  Attempt {attempt_for_healthy}:")
                log(f"    IXI:   {healthy_path.name}")
                log(f"    BraTS: {label_path.name}")
                log(f"    Case:  {case_dir}")
                ...
                base_row = {
                    ...
                    "attempt_for_healthy": attempt_for_healthy,
                    "label_global_index": label_global_index,
                ...
                if case_is_complete(case_dir) and not args.overwrite:
                    log("    Existing complete case found; marking as skipped_existing and accepting this pair.")
                    skipped_existing += 1
                    success_for_this_ixi = True
                ...
                try:
                    ...
                    x, y, z = random_center_tumour_voxel(
                        healthy_scan=healthy_scan,
                        original_label=original_label,
                    )
                    if x is None:
                        raise RuntimeError("No valid random placement found after 1000 tries.")
                    ...
                    metadata = {
                        ...
                        "healthy_index": int(healthy_idx),
                        "healthy_path": str(healthy_path),
                        "healthy_stem": healthy_stem,
                        "label_global_index": int(label_global_index),
                        "label_path": str(label_path),
                        "label_stem": label_stem,
                        "case_dir": str(case_dir),
                        ...
                        "attempt_for_healthy": int(attempt_for_healthy),
                        **meta,
                    }
                    ...
                    failed_mask_attempts += 1
                    err = repr(e)
                    log(f"    FAILED with mask {label_path.name}: {err}")
                    log("    This BraTS mask is consumed and will NOT be reused.")
                    log(traceback.format_exc())


                    fail_dir = failed_attempt_dir_for(
                        healthy_path=healthy_path,
                        label_path=label_path,
                        attempt_idx=attempt_for_healthy,
                    ...
                    fail_meta = {
                        ...
                        "label_stem": label_stem,
                        "case_dir": str(case_dir),
                        "failed_attempt_dir": str(fail_dir),
                        "error": err,
                    ...
                    save_json(fail_meta, fail_dir / "metadata_failed.json")


                    row = {
                        **base_row,
                        "case_dir": str(fail_dir),
                        "status": "failed",
                        "error": err,
                        "center": "",
                        "crop_bounds": "",
                    ...
                    append_csv_row(attempts_csv, ATTEMPT_FIELDS, row)
                    append_csv_row(failed_csv, FAILED_LABEL_FIELDS, row)
        ...
        log(f"Failed IXIs with no successful output: {failed_ixis}")
        log(f"Failed mask attempts consumed: {failed_mask_attempts}")
        log(f"Unused masks remaining: {len(label_files) - label_cursor}")
        log("")
        log(f"Attempts CSV: {attempts_csv}")
        log(f"Success pairings CSV: {successes_csv}")
        log(f"Failed attempts CSV: {failed_csv}")
        log(f"Log file: {log_path}")
