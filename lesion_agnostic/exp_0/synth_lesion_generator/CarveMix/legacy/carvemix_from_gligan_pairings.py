...
Example:
py -3.10 carvemix_from_gligan_pairings.py `
  ...
  --output-dir "data\generated_carvemix_matched" `
  --target-total 550 `
...
def path_contains_id(path: Path, wanted: str) -> bool:
    if wanted is None:
        return False
    hay = normalize_id_text(path.name)
...
def parse_pairings_csv(path: Path) -> list[dict]:
    ...
    for idx, row in df.iterrows():
        ...
        # Preferred explicit columns if available
        ...
        # Fallback: parse from case_dir like:
        # data\IXI_GLI\IXI131-HH-1527-T1__BraTS-GLI-02708-100
    ...
    # Deduplicate by IXI, keeping first occurrence.
    # Your GliGAN successful pairings should already be unique, but this protects you.
    seen_ixi = set()
    unique_pairs = []
    for p in pairs:
...
def generate_one(
    ...
    if center is None:
        return False, {**meta_out, "failure_reason": "could_not_place_tumor"}
...
def choose_unused_random_item(library_items: list[Path], attempted_items: set[str], rng_py: random.Random) -> Optional[Path]:
...
def attempt_until_success_for_healthy(
    ...
    for fallback_idx in range(max_fallbacks):
        item = choose_unused_random_item(library_items, attempted_items | {str(p) for _, p in candidate_items}, rng_py)
        if item is None:
            break
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="CarveMix generation using GliGAN pairings as preferred pairs."
    )
    ...
    for pair_idx, pair in enumerate(gligan_pairs, start=1):
        ...
        if preferred_item is None:
            logging.warning(
                ...
                pair_idx, len(gligan_pairs), ixi_id, gli_id
            )

        logging.info(
            "[%d/%d] Matched phase | IXI=%s | preferred GLI=%s | preferred item=%s",
            pair_idx, len(gligan_pairs), ixi_id, gli_id, preferred_item.name if preferred_item else "NONE"
        )
    ...
    if count_after_matched < args.target_total and args.extra_random_if_under > 0:
        ...
        for h_idx, healthy_path in enumerate(unused_healthy, start=1):
            ...
            logging.info(
                ...
                h_idx, len(unused_healthy), ixi_id, extra_successes, extra_n
            )
