...
"""
CarveMix generation with optional GliGAN pairings.

Modes:
1. CSV mode:
   - Use --gligan-pairings-csv successful_ixi_brats_pairings.csv
   - Pairs IXI and BraTS according to the CSV.

2. No-CSV mode:
   - Omit --gligan-pairings-csv
   - Pairs healthy MRI files and tumor library items one-by-one after sorting:
       healthy_files[0] + library_items[0]
       healthy_files[1] + library_items[1]
       ...

This version preserves original CarveMix-style lesion location.

Example with CSV:
python carvemix_from_gligan_pairings_original_location.py \
  --healthy-dir /path/to/IXI \
  --library-dir /path/to/BraTS_Masks \
...
python3.10 carvemixfig.py \
    --healthy-dir cm_fig_in \
    --library-dir cm_fig_lib \
    --output-dir "generated_carvemix_fig2" \
    --seed 42
"""
...
def normalize_id_text(x: str) -> str:
    ...
    x = x.replace("_synthetic_t1", "")
    x = x.replace("_synthetic_seg", "")
    x = x.replace("-seg", "")
    x = x.replace("_seg", "")
    return x
...
def make_one_by_one_pairs(healthy_files: list[Path], library_items: list[Path]) -> list[dict]:
    healthy_sorted = sorted(healthy_files)
    tumor_sorted = sorted(library_items)

    n_pairs = min(len(healthy_sorted), len(tumor_sorted))
    pairs = []

    for idx in range(n_pairs):
        h = healthy_sorted[idx]
        t = tumor_sorted[idx]

        ixi = extract_ixi_id(h.name)
        gli = extract_gli_id(t.name)

        if ixi is None or gli is None:
            logging.warning("Skipping unparseable one-by-one pair: healthy=%s tumor=%s", h.name, t.name)
            continue
        ...
        pairs.append(
            {
                ...
                "raw_row": {
                    "healthy_path": str(h),
                    "library_item_path": str(t),
                    "pairing_mode": "one_by_one_no_csv",
                },
            }
        )
...
def place_at_original_location(
    ...
    if patch.shape != expected_shape or patch_mask.shape != expected_shape:
        raise RuntimeError(
            f"Patch shape does not match bbox size. "
            f"patch.shape={patch.shape}, mask.shape={patch_mask.shape}, bbox_shape={expected_shape}"
        )
...
def output_stem_for(healthy_path: Path, item_path: Path) -> str:
    ...
    h = h.replace("_preprocessed", "").replace("_brain_n4_rigid", "")
    h = h.replace("_synthetic_t1", "")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="CarveMix generation using optional GliGAN pairings or one-by-one sorted pairing."
    )

    parser.add_argument("--healthy-dir", type=Path, required=True)
    parser.add_argument("--library-dir", type=Path, required=True)
    parser.add_argument("--gligan-pairings-csv", type=Path, default=None)
...
def main() -> None:
    ...
    if args.gligan_pairings_csv is not None:
        gligan_pairs = parse_pairings_csv(args.gligan_pairings_csv)
        pairing_mode = "csv"
    else:
        gligan_pairs = make_one_by_one_pairs(healthy_files, library_items)
        pairing_mode = "one_by_one_no_csv"
    ...
    logging.info("Pairing mode: %s", pairing_mode)
    ...
    logging.info("Pairings loaded/generated: %d", len(gligan_pairs))
    ...
    for pair_idx, pair in enumerate(gligan_pairs, start=1):
        ...
        if ixi_id in used_healthy_success:
            logging.warning("[%d/%d] IXI already successfully used, skipping: %s", pair_idx, len(gligan_pairs), ixi_id)
            continue

        healthy_path = healthy_index.get(ixi_id)

        if healthy_path is None:
            logging.warning("[%d/%d] Missing healthy MRI for IXI=%s", pair_idx, len(gligan_pairs), ixi_id)
            failed_ixi.append(
                {
                    "source_phase": pairing_mode,
                    "ixi_id": ixi_id,
        ...
        logging.info(
            "[%d/%d] Pairing phase | IXI=%s | preferred GLI=%s | preferred item=%s",
            pair_idx,
            len(gligan_pairs),
            ixi_id,
            gli_id,
    ...
    logging.info("Pairing phase done. Successes=%d", count_after_matched)
    ...
    if count_after_matched < args.target_total and args.extra_random_if_under > 0:
        ...
        unused_healthy = [
            p
            for p in healthy_files
            if extract_ixi_id(p.name) is not None and extract_ixi_id(p.name) not in used_healthy_success
        ]
    ...
    summary = {
        ...
        "gligan_pairings_csv": str(args.gligan_pairings_csv) if args.gligan_pairings_csv is not None else None,
        ...
        "num_pairings_loaded_or_generated": int(len(gligan_pairs)),
