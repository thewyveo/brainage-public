"""
SFCN-style preprocessing for multiple T1-weighted MRIs in a folder.

Faithful preprocessing steps from the SFCN / UK Biobank description:
1. Brain extraction with FSL BET
2. Bias field correction with FSL FAST
3. Affine registration to MNI152 standard space with FSL FLIRT

This version runs FSL THROUGH WSL from normal Python.

Example
-------
py -3.10 preprocessing/preprocess_sfcn_batch_original.py --input-dir X:\data\raw\BraTS\ --output-dir X:\data\preprocessed\original\BraTS\ --mni X:\data\MNI152_T1_1mm_Brain.nii --name-filter t1n
"""
...
def run_wsl_capture(cmd_str: str) -> str:
...
def copy_windows_file_to_wsl(win_path: Path, wsl_dir: str, new_name: str) -> str:
    src = windows_to_wsl_path(win_path)
    dst = f"{wsl_dir}/{new_name}"
    run_wsl_cmd(f'cp {bash_quote(src)} {bash_quote(dst)}')
    return dst


def copy_wsl_file_to_windows(wsl_path: str, win_path: Path):
    win_path.parent.mkdir(parents=True, exist_ok=True)
    dst = windows_to_wsl_path(win_path)
    run_wsl_cmd(f'cp {bash_quote(wsl_path)} {bash_quote(dst)}')


def bash_quote(path_str: str) -> str:
    return "'" + path_str.replace("'", "'\"'\"'") + "'"
...
def ensure_wsl_and_fsl():
    ...
    # Check that FSL tools are available inside WSL
    test_cmd = "which bet && which fast && which flirt"
    try:
        subprocess.run(["wsl", "bash", "-lc", test_cmd], check=True, capture_output=True, text=True)
    except subprocess.CalledProcessError as e:
        raise RuntimeError(
            "Could not find FSL tools inside WSL. Make sure bet/fast/flirt work in your WSL shell first."
        ) from e


def run_bet(inp: Path, out: Path):
    """
    Brain extraction with FSL BET via WSL.
    """
    ...
    cmd = f'bet "{inp_wsl}" "{out_wsl}" -R -f 0.5 -g 0'
    run_wsl_cmd(cmd)
...
def run_affine_flirt_to_mni(inp: Path, mni: Path, out: Path, mat_out: Path | None = None):
    ...
    IMPORTANT:
    This function should only be used with temp/local paths that WSL can access,
    not directly with X: or other Windows-only output targets.
    """
    out.parent.mkdir(parents=True, exist_ok=True)
...
def preprocess_single_image(
    ...
    with tempfile.TemporaryDirectory(prefix="sfcn_preproc_") as tmpdir_str:
        ...
        # Copy input + MNI locally using PYTHON
        ...
        # IMPORTANT: FLIRT also writes into temp now
        ...
        # 1. BET
        run_bet(local_input, brain_out)

        # 2. FAST
        run_fast_bias_correction(brain_out, biascorr_out)

        # 3. FLIRT writes to temp, not X:
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Faithful SFCN-style batch preprocessing with FSL via WSL."
    )
    parser.add_argument(
        "--input-dir",
        type=Path,
    ...
    parser.add_argument(
        "--keep-intermediate",
        action="store_true",
        help="Keep BET/FAST/FLIRT intermediate files."
    )
    parser.add_argument(
        "--name-filter",
        type=str,
