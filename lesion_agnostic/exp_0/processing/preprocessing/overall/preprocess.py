...
py -3.10 exp_0\processing\preprocessing\overall\preprocess2.py `
    --profile gligan `
    --input-dir data\raw\IXI-T1 `
    --output-dir data\preprocessed\GliGAN\IXI `
    --mni data\MNI152_T1_1mm_Brain.nii `
    --name-filter T1 `
    --workers 1

4.10.26 friday - gligan with scaled tumor - preprocessed for BANeXt
py -3.10 exp_0\processing\preprocessing\overall\preprocess2.py `
    --profile brainagenext `
    --input-dir exp_0\synth_lesion_generator\GliGAN\data\SCALEDFLAT_generated_t1_gligan `
    --output-dir data\preprocessed\BrainAgeNeXt\IXI_scaled `
    --mni data\MNI152_T1_1mm_Brain.nii `
    --workers 1 `
    --refresh-cache

4.11.26 - gligan with scaled tumor - preprocessed for BANeXt - scaled 2 (bigger data)
py -3.10 exp_0\processing\preprocessing\overall\preprocess2.py `
    --profile brainagenext `
    --input-dir exp_0\synth_lesion_generator\GliGAN\data\SCALED2FLAT_generated_t1_gligan `
    --output-dir data\preprocessed\BrainAgeNeXt\IXI_scaled2 `
    --mni data\MNI152_T1_1mm_Brain.nii `
    --workers 1 `
    --refresh-cache

4.13.6 - carvemix w guizard - brainagenext profile preprocessing for library build
py -3.10 exp_0\processing\preprocessing\overall\preprocess2.py `
    --profile brainagenext `
    --input-dir data\library\CM_BraTS_Masks `
    --output-dir data\preprocessed\BrainAgeNeXt\BraTS_T1_BANeXt `
    --mni data\MNI152_T1_1mm_Brain.nii `
    --workers 1 `
    --name-filter t1n `
    --refresh-cache
...
py -3.10 exp_0\processing\preprocessing\overall\preprocess2.py `
    --profile brainagenext `
    --input-dir exp_0\synth_lesion_generator\GliGAN\data\FAITHFULFLAT_generated_t1_gligan `
    --output-dir data\preprocessed\BrainAgeNeXt\GliGAN_FAITHFUL `
    --mni data\MNI152_T1_1mm_Brain.nii `
    --workers 1 `
    --refresh-cache
...
py -3.10 exp_0\processing\preprocessing\overall\preprocess2.py `
    --profile custom `
    --input-dir data\raw\IXI-T1 `
    --output-dir data\preprocessed\UNA_USB `
    --mni data\MNI152_T1_1mm_Brain.nii `
    --workers 1 `
    --refresh-cache `
    --enable-synthstrip `
...
py -3.10 exp_0\processing\preprocessing\overall\preprocess2.py `
    --profile custom `
    --input-dir data\GLI_USB `
    --output-dir data\BNX_GLI_USB_PREP `
    --mni data\MNI152_T1_1mm_Brain.nii `
    --workers 1 `
    --refresh-cache `
    --disable-reorient `
...
- SFCN faithful
- SFCN modified
- GliGAN / BraTS-like healthy-brain preprocessing
...
--profile gligan
    Whole-volume BraTS-like preparation for healthy scans:
    1. SynthStrip
    2. ANTs affine registration
    3. resample to 1 mm isotropic
    4. center crop/pad to 182 x 218 x 182


    NOTE:
    The GliGAN generator-specific 96x96x96 crop / noise insertion / [-1,1]
    normalization should be done later in the tumor insertion code, not here.
...
def run_resample_spacing(
    ...
    copied = maybe_copy_disabled(inp, out, enabled, "ANTs resample spacing")
    if copied is not None:
        return copied
    ...
    ensure_parent(out)
    img = ants.image_read(str(inp))
    resampled = ants.resample_image(img, resample_params=spacing, use_voxels=False, interp_type=interp_type)
...
def run_center_crop_or_pad(
    ...
    copied = maybe_copy_disabled(inp, out, enabled, "center crop/pad")
    if copied is not None:
        return copied
    ...
    out_img = nib.Nifti1Image(current.astype(np.float32), img.affine, img.header)
...
def zscore_normalize_nonzero(inp_nii: Path, out_nii: Path, enabled: bool, eps: float = 1e-8) -> Path:
    copied = maybe_copy_disabled(inp_nii, out_nii, enabled, "z-score normalization on nonzero voxels")
    if copied is not None:
        return copied
    ...
    if vals.size == 0:
        raise ValueError(f"No nonzero voxels available for z-score normalization in {inp_nii}")
...
def apply_profile_defaults(args: argparse.Namespace) -> argparse.Namespace:
    profile_defaults = {
        "joos": {
            ...
            "enable_crop": False,
            "crop_shape": (240, 240, 155),
            "enable_normalize_1_99": True,
            "enable_zscore_nonzero": False,
            "save_npy": True,
            "output_suffix": "",
        },
        "brainagenext": {
        ...
        "gligan": {
            ...
            "enable_bet": False,
            "enable_fast": False,
            "registration_backend": "ants_affine",
            "enable_resample": True,
            "target_spacing": (1.0, 1.0, 1.0),
            "enable_crop": True,
            "crop_shape": (182, 218, 182),
            "enable_normalize_1_99": False,
            "enable_zscore_nonzero": False,
            "save_npy": False,
            "output_suffix": "_preprocessed",
    ...
    if args.enable_normalize_1_99 and args.enable_zscore_nonzero:
        raise ValueError("Choose only one intensity normalization method: 1-99 normalization or z-score normalization.")
...
def process_image(
    ...
    with tempfile.TemporaryDirectory(prefix="universal_preproc_") as tmpdir_str:
        ...
        p_resamp = tmpdir / f"{stem}_resampled.nii.gz"
        p_crop = tmpdir / f"{stem}_cropped.nii.gz"
        ...
        if reg_backend == "none":
            ...
            current = run_ants_registration(
                current,
                local_mni,
                p_reg,
                True,
                transform_type="Rigid",
                transform_prefix=tmpdir / f"{stem}_ants_rigid"
            )
            ...
            current = run_ants_registration(
                ...
                transform_prefix=tmpdir / f"{stem}_ants_affine"
            )
        ...
        # 7. Resample
        current = run_resample_spacing(
            current,
            p_resamp,
            args.enable_resample,
        ...
        # 8. Crop / pad
        current = run_center_crop_or_pad(
            current,
            p_crop,
            args.enable_crop,
            target_shape=tuple(args.crop_shape),
            pad_value=0.0,
        )


        # 9. Intensity normalization
        if args.enable_zscore_nonzero:
            current = zscore_normalize_nonzero(current, p_norm, True)
        else:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--profile", type=str, required=True,
                        choices=["joos", "brainagenext", "sfcn_faithful", "sfcn_modified", "gligan", "custom"],
    ...
    parser.add_argument("--enable-resample", dest="enable_resample", action="store_true", default=None)
    parser.add_argument("--disable-resample", dest="enable_resample", action="store_false")
    parser.add_argument("--target-spacing", type=float, nargs=3, default=None,
                        metavar=("SX", "SY", "SZ"),
                        help="Target voxel spacing, e.g. --target-spacing 1 1 1")


    parser.add_argument("--enable-crop", dest="enable_crop", action="store_true", default=None)
    parser.add_argument("--disable-crop", dest="enable_crop", action="store_false")
    parser.add_argument("--crop-shape", type=int, nargs=3, default=None,
                        metavar=("NX", "NY", "NZ"),
                        help="Target crop/pad shape, e.g. --crop-shape 182 218 182")
    ...
    parser.add_argument("--enable-zscore-nonzero", dest="enable_zscore_nonzero", action="store_true", default=None)
    parser.add_argument("--disable-zscore-nonzero", dest="enable_zscore_nonzero", action="store_false")
...
# NAME OF SAVED FILE: t1n_preprocessed.nii.gz
# ^^^ DEFINED IN LINE 1375: shutil.copy2(current, final)
# where do i hard code it to be t1n_preprocessed.nii.gz?
# answer: in the process_image function, in the line: shutil.copy2(current, final)