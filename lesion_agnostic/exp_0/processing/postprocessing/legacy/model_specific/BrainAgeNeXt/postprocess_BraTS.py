...
def main():
    ...
    predictions_path = script_dir.parent / "data" / "predictions" / "BraTS_24_predictions.csv"
    ...
    meta_df = load_table(metadata_path)
    ...
    # keep only subjects that were actually predicted
    ...
    if "Patient's Age" not in merged_df.columns:
        raise ValueError("Column \"Patient's Age\" not found after merge.")

    if "Predicted_Brain_Age" not in merged_df.columns:
        raise ValueError("Column \"Predicted_Brain_Age\" not found in predictions CSV.")

    if "Brain_Age_Difference" not in merged_df.columns:
    ...
    # Change these names if your exact metadata columns differ
    feature_columns = [
        "Site",
