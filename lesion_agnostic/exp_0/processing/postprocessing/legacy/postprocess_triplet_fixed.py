...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional. Label of the synthetic-tumor set among --set entries, e.g. BNX_GLI. "
            "If provided, the script creates healthy/synthetic/inpainted triplet BAG plots."
        ),
    )
    parser.add_argument(
        "--triplet-inpaint-label",
        action="append",
        default=None,
        help=(
            "Optional. Label(s) among --set entries to treat as inpainted sets for triplet plots. "
            "Can be repeated. If omitted, every set except --triplet-synth-label is treated as an inpainted set."
        ),
    )
...
def main() -> None:
    ...
    # Keep all normalized set DataFrames by label so triplet plots can be built
    # after every --set has been loaded. This matters because --set has no
    # explicit role distinction; labels define the roles.
    ...
    for label, path in set_specs:
        ...
        # Standard paired outputs:
        # baseline healthy vs this set.
        # This is kept exactly general; this set may be synthetic tumor OR an inpainted set.
        run_paired_outputs(
    ...
    # Optional triplet plots:
    # healthy baseline -> synthetic tumor reference -> each inpainted set.
    #
    # Because --set does not distinguish synthetic tumor from inpainted inputs,
    # the user explicitly names which --set label is the synthetic-tumor reference.
    # Example:
    #   --triplet-synth-label BNX_GLI
    #
    # If --triplet-inpaint-label is omitted, all other --set labels are treated
    # as inpainted candidates.
    if args.triplet_synth_label is not None:
        synth_label = args.triplet_synth_label

        if synth_label not in set_dfs:
            raise ValueError(
                f"--triplet-synth-label '{synth_label}' was not found among --set labels. "
                f"Available set labels: {list(set_dfs.keys())}"
            )

        synth_ref_df = set_dfs[synth_label]

        if args.triplet_inpaint_label:
            inpaint_labels = list(args.triplet_inpaint_label)
        else:
            inpaint_labels = [label for label in set_dfs.keys() if label != synth_label]

        for inpaint_label in inpaint_labels:
            if inpaint_label not in set_dfs:
                raise ValueError(
                    f"--triplet-inpaint-label '{inpaint_label}' was not found among --set labels. "
                    f"Available set labels: {list(set_dfs.keys())}"
                )

            if inpaint_label == synth_label:
                print(f"Skipping triplet plot for synthetic reference itself: {inpaint_label}")
                continue

            print(
                f"Creating triplet BAG plot: baseline={baseline_label}, "
                f"synth={synth_label}, inpaint={inpaint_label}"
            )

            triplet = build_triplet_df(
                base_df=baseline_df,
                synth_df=synth_ref_df,
                inpaint_df=set_dfs[inpaint_label],
            )

            triplet_out_dir = args.output_dir / sanitize(inpaint_label)
            triplet.to_csv(triplet_out_dir / "triplet_comparison_against_synth_reference.csv", index=False)

            bag_triplet_shift_plot_closer_to_baseline(
                triplet,
                triplet_out_dir / f"bag_triplet_shift_closer_to_healthy_vs_{sanitize(synth_label)}.png",
            )
